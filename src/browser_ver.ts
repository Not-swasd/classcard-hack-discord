...
if (!fs.existsSync("./config.json"))
    fs.writeFileSync(
        "./config.json",
        JSON.stringify({ id: "", password: "" }, null, 4)
    );
...
let response = await axios
    .post(
        ...
        transformRequest({
            sess_key: cookies.find(
                (cookie: playwright.Cookie) => cookie.name === "ci_session"
            )!.value,
            redirect: "",
            login_id: config.id,
...
if (!response.data || response.data.result !== "ok") {
    console.error(
        chalk.redBright(`\r[CCH] 로그인 실패. (msg: ${response.data.msg}) `)
    );
    process.exit(1);
}
...
const evalContents = {
    //https://github.com/Nua07/classcardHack/ 참고.
    std: `(() => {
            arr_send_card_idx = [0];
            arr_send_score = [0];
            ...
            jQuery(function ($) {
                let send_data = [ ${
                    game === "Match"
                        ? `ggk.d(100, 0)`
                        : game === "Scramble"
                        ? "ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0), ggk.d(50, 0)"
                        : ""
                } ];
                for (var i = 0; i < (${score} / ${
            ["Match", "Scramble"].includes(game) ? "100" : "10"
        }); i++) send_data.push(ggk.d(${
            ["Match", "Scramble"].includes(game) ? "100" : "10"
        }, 1));
...
page.route("**/*", (route: playwright.Route, req: playwright.Request) => {
    if (req.url().includes("JsError")) return route.abort();
    if (req.url().includes("match.js"))
        route.fulfill({
            status: 200,
            body: fs.readFileSync("./ccfiles/match.js", "utf8"),
        });
    else if (req.url().includes("crash.js"))
        route.fulfill({
            status: 200,
            body: fs.readFileSync("./ccfiles/crash.js", "utf8"),
        });
    else if (req.url().includes("match_sentence.js"))
        route.fulfill({
            status: 200,
            body: fs.readFileSync("./ccfiles/match_sentence.js", "utf8"),
        });
    else if (req.url().includes("class_test.js"))
        route.fulfill({
            status: 200,
            body: fs.readFileSync("./ccfiles/class_test.js", "utf8"),
        });
    else if (
        !req.url().startsWith("https://www.classcard.net") &&
        !req.url().startsWith("https://b.classcard.net")
    )
        route.abort();
    else route.continue();
});
page.on("load", async (p: playwright.Page) => {
    ...
    while (
        ing ||
        (!!setid && setStatus["Memorize"] != 0 && !setStatus["Memorize"])
    ) {
        await sleep(300);
        if (!!setid && !setStatus["Memorize"])
            await getSetInfo(true).then((r) => {
                if (!!r) setStatus = r;
            });
    }
    ...
    if (p.url().includes("/ClassTest"))
        page.evaluate(evalContents["testTimeLock"]).catch(() => false);
    let name = p.url().split(".net/")[1].split("/")[0] as
        | "Memorize"
        | "Recall"
        | "Spell"
        | "Match"
    ...
    if (["Memorize", "Recall", "Spell", "Match", "Crash"].includes(name)) {
        console.clear();
        ing = name;
        if (name === "Match" && setStatus.type === "sentence")
            name = "Scramble";
        console.info(
            chalk.yellowBright(
                `[CCH] ${names[name]} 페이지에 접속하셨습니다. 잠시만 기다려주세요.`
            )
        );
        if (p.url().includes("/Match") || p.url().includes("/Crash")) {
            ...
            while (score == 0) {
                ...
                if (
                    isNaN(score) ||
                    score < (["Match", "Scramble"].includes(name) ? 100 : 10) ||
                    score > 990000 ||
                    score % (["Match", "Scramble"].includes(name) ? 100 : 10) !=
                        0
                ) {
                    score = 0;
                    console.error(
                        chalk.redBright("[CCH] 올바르지 않은 점수입니다.")
                    );
                    await sleep(700);
                    continue;
                }
            ...
            let result =
                (await page
                    .evaluate(evalContents["gameCustomScore"](name, score))
                    .catch(() => false)) === undefined;
            console.info(
                chalk[result ? "greenBright" : "redBright"](
                    "[CCH] " +
                        (result ? "성공" : "실패") +
                        "! 종료 버튼을 누르거나 세트페이지로 이동하세요."
                )
            );
        } else {
            name = name as "Memorize" | "Recall" | "Spell";
            let result =
                (await page.evaluate(evalContents.std).catch(() => false)) ===
                undefined;
            if (["Memorize", "Recall", "Spell"].includes(name)) {
                ...
                if (
                    !temp ||
                    temp[name] !==
                        Number(setStatus[name].toString().slice(0, -2) + "00") +
                            100
                )
                    result = false;
...
async function logSetInfo() {
    ...
    console.info(
        chalk.greenBright(
            `[CCH] *오류가 있어 제대로 작동하지 않습니다.* 세트 이름: ${setName}\n      세트 ID: ${setid}\n      암기: ${setStatus.Memorize}%\n      리콜: ${setStatus.Recall}%\n      스펠: ${setStatus.Spell}%`
        )
    );
    console.info(
        chalk.yellowBright(
            "[CCH] 지원 목록: " + Object.values(names).join(", ")
        )
    );
}
...
async function getSetInfo(first: boolean) {
    let response = await axios
        .get(`https://www.classcard.net/set/${setid}`, {
            headers: {
                Cookie:
                    "ci_session=" +
                    cookies.find(
    ...
    return {
        Memorize: 0, //Number(response.data.split('mem-total-rate" data-rate="')[1].split('"')[0].trim())
        Recall: 0, //Number(response.data.split('recall-total-rate" data-rate="')[1].split('"')[0].trim())
        Spell: 0, //Number(response.data.split('spell-total-rate" data-rate="')[1].split('"')[0].trim())
        type: "ㅁㄹ",
    };
}
